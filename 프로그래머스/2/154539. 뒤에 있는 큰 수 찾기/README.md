...
<h3>풀이</h3>
<p>스택을 사용하는 문제. 우선 배열의 뒤에서부터 뒷큰수를 찾는 것이 중요하다. 만약 스택을 확인했을 때, 비어있다면 뒷큰수가 없는 것이므로 -1이 된다. 비어있지 않은 경우라면, 스택의 top이 비교 숫자보다 작다면, 가장 최근에 스택에 추가된 top이 비교 숫자의 뒷큰수가 된다. 만약 작은 경우는 비교 숫자보다 큰 top이 나올때까지 계속해서 pop을 해주어 뒷큰수를 찾는다.  </p>
