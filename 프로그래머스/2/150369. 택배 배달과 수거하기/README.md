...
<hr>
<h2> 풀이 </h2>
<p>1. 기본적으로 배열 맨 뒤부터 탐색을 시작한다. 뒤에서부터 탐색하는 이유는 배달 및 회수를 하는 과정에서 맨 뒤부터 확인을 해야, 해당 지점까지 배달하고 나서 회수할 수 있는 양을 계산할 수 있게 되기 때문이다.</p>
<p>2. 탐색되는 집의 배달량과 회수량을 뺀 값을 d,p에 저장한다. 이를 통해 해당 집에 대해서 처리해야하는 배달량과 회수량이 저장된다.</p>
<p>3. 그 다음 반복문을 통해, d와 p가 양수가 될 때까지 cap을 더해주며 cnt를 센다. 이렇게 하는 이유는 한 번 해당 집을 찍고 돌아오는 사이클을 진행할 때마다 cap만큼의 배달량과 회수량을 처리할 수 있기 때문에 cap을 더해주는 것이고, 이러한 사이클 횟수를 카운팅해서 cnt에 저장해둠으로써, 해당 집에 대한 작업 처리를 하는데 필요한 최소한의 왕복 횟수를 구할 수 있는 것.</p>
<p>4. 만일 배달량과 회수량이 둘 다 0이상이 되면, 해당 집에 대한 작업 처리량이 모두 마무리 된 것이므로 해당 집까지의 거리 * 왕복이니까 2 * 왕복 횟수 만큼 구해서 이동량에 추가해주면 되는 것.</p>
<p>만일 어떠한 집에 대한 배달량과 작업량을 빼주는 단계 이후에도 d와 p가 이미 둘다 0이상인 경우는 해당 집보다 뒤에 있는 집에 대한 작업을 처리하기 위해 왔다갔다 했던 과정에서 이미 작업량이 처리된 상황임을 의미하게 된다.</p>
