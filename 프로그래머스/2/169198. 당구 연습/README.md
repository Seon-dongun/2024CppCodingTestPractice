...
<h2> 개인 코멘트 </h2>
<p>핵심은 도착점을 상,하,좌,우로 대칭이동 시켜서 시작점과 도착점을 직선의 형태로 그어 해당 길이를 구해내는 것임. 시작점과 도착점이 x,y축 모두 동일선 상에 있지 않은 일반적인 경우는 원쿠션이 가능한 방법이 상,하,좌,우로 가능하여 모두 확인해봐야 하지만, 만약에 x축으로 시작점과 도착점이 동일선 상에 있는 경우는 도착점의 y좌표가 시작점의 y좌표보다 위에 있으면 위쪽 쿠션은 불가능하고 아래에 있는 경우는 아래 쿠션이 불가능하다. 또한 y축으로 시작점과 도착점이 동일선 상에 있는 경우는 도착점의 x좌표가 시작점의 x좌표보다 오른쪽에 있으면 오른쪽 쿠션이 불가능하고, 왼쪽에 있으면 왼쪽 쿠션이 불가능하다. 이러한 조건을 적절히 활용해서 가능한 원쿠션의 경로중에서 가장 작은 경로값을 찾으면 되는 것.</p>
